...
test('strategy switching lives in the header and redraws dashboard context', () => {
  ...
  assert.match(js, /\{ id: 'structure', label: 'Struktura' \}/)
  assert.match(js, /const structureReferenceFacts = \(entry\) =>/)
  assert.match(js, /item\.externalTrend/)
  assert.match(js, /item\.externalPivots/)
  assert.match(js, /pivotReference\.source/)
  ...
  assert.match(js, /priceActionDecisionTimeframe = chartColumn\.id/)
  assert.match(js, /priceActionDecisionTimeframe = chartColumn\.id[\s\S]*?renderDecision\(\)/)
  ...
  assert.match(js, /const directionalPriceRange = \(\{ low, high, direction = null \} = \{\}\) =>/)
  assert.match(js, /return direction === 'up'/)
  assert.match(js, /direction: structureDirection\(entry\)/)
  ...
  assert.match(js, /'TP1 \/ TP2'/)
  assert.match(js, /quotePrice\(order\.tp1\)/)
  assert.match(js, /Number\.isFinite\(order\.tp2\)/)
  assert.match(js, /'struktura'/)
  assert.match(js, /TP2: bez platné \$\{zoneType\} zóny za TP1/)
  ...
  assert.match(js, /realizedStatsForTrades/)
  assert.match(js, /strategyId \? 'Realizované P\/L PA-1' : 'Realizované P\/L'/)
  assert.match(js, /renderPortfolioTiles\(box, \{ strategyId: 'price-action-structure-v1' \}\)/)
  ...
  assert.match(css, /structure-reference-facts/)
...
test('asset tickers open a timeframe price chart with supply and demand zones', () => {
  ...
  assert.match(js, /const storedContext = item\?\.structure\?\.contextPivots/)
  assert.match(js, /Potvrzený kontext před aktivní vlnou/)
  ...
  assert.match(js, /const candleWidth = Math\.max\(2, Math\.min\(12, candleSpacing \* 0\.62\)\)/)
  assert.match(js, /const minimumCandleBodyHeight = 4/)
  assert.match(js, /const bodyHeight = Math\.max\(minimumCandleBodyHeight, Math\.abs\(closeY - openY\)\)/)
  assert.match(js, /const bodyY = \(\(openY \+ closeY\) \/ 2\) - bodyHeight \/ 2/)
  assert.match(js, /const externalEma = item\?\.externalTrend\?\.ema/)
  assert.match(js, /asset-external-\$\{key\}/)
  assert.match(css, /asset-external-ema20/)
  assert.match(css, /asset-external-ema50/)
  assert.match(css, /asset-external-structure-path/)
  assert.match(css, /asset-external-structure-active/)
  assert.doesNotMatch(js, /asset-external-pivot/)
  assert.doesNotMatch(css, /asset-external-pivot/)
  assert.doesNotMatch(js, /H flat/)
  assert.match(js, /assetChartVisibleCandleCount/)
  assert.match(js, /'1h': 480/)
  assert.match(js, /const maxHistoryOffset = Math\.max\(0, allCandles\.length - assetChartVisibleCandleCount\)/)
  assert.match(js, /const candleEnd = allCandles\.length - assetChartHistoryOffset/)
  assert.match(js, /candles = allCandles\.slice\(candleEnd - assetChartVisibleCandleCount, candleEnd\)/)
  ...
  assert.match(js, /const resetAssetChartYScale = \(\) =>/)
  assert.match(js, /historyRange\.oninput = \(\) =>[\s\S]*?resetAssetChartYScale\(\)/)
  assert.match(js, /assetChartVisibleCandleCount = nextVisibleCount[\s\S]*?resetAssetChartYScale\(\)/)
  ...
  assert.match(js, /assetChartPan/)
  assert.match(js, /svg\.ondblclick = \(event\) =>/)
  assert.match(js, /asset-chart-pan-enabled/)
  ...
  assert.match(css, /\.asset-price-chart \{ overflow: hidden; isolation: isolate; \}/)
  assert.match(css, /asset-chart-pan-enabled/)
...
test('the API refuses stale PA-1 runners before they overwrite current structure', () => {
  assert.match(api, /const MIN_PRICE_ACTION_MATRIX_SCHEMA = 56/)
