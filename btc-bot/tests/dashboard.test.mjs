...
test('strategy tab renders the price-action structure matrix', () => {
  ...
  assert.match(js, /renderStructureDetail/)
...
test('strategy switching lives in the header and redraws dashboard context', () => {
  ...
  assert.match(js, /text: assetPriceLabel\(asset\.symbol, item\?\.price\)/)
  ...
  assert.match(js, /text: 'Tvorba struktury'/)
...
test('asset tickers open a timeframe price chart with supply and demand zones', () => {
  ...
  assert.match(js, /assetChartHistoryOffset/)
  assert.match(js, /visibleCandleCount = Math\.min\(60, allCandles\.length\)/)
