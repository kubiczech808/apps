...
const RESOLVE_TRADES_TIMEOUT_MS = 3e4;
const RESOLVE_TRADES_POLL_INTERVAL_MS = 250;
const FAILED_TRADE_STATUS = "FAILED";
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const isTradeResolved = (trade) => trade.status.toUpperCase() === FAILED_TRADE_STATUS || Boolean(trade.transaction_hash && trade.transaction_hash.length > 0);
var ClobClient = class {
	...
	*/
	async postOrder(order, orderType = require_clob.OrderType.GTC, postOnly = false, deferExec = false) {
		...
		const response = this.throwIfError(res);
		if (deferExec) return response;
	...
	async postOrders(args, postOnly = false, deferExec = false) {
		...
		if (this._isOrderVersionMismatch(res)) await this.resolveVersion(true);
		const responses = this.throwIfError(res);
		if (deferExec || !Array.isArray(responses)) return responses;
		return Promise.all(responses.map((response) => this.resolveTransactionsHashes(response)));
	}
	async waitForResolvedTrades(tradeIDs) {
		...
		for (;;) {
			...
			const pages = await Promise.all(pending.map((id) => this.getTrades({ id }, true).catch(() => [])));
			for (const trades of pages) for (const trade of trades) if (isTradeResolved(trade) && requestedIDs.has(trade.id)) resolved.set(trade.id, trade);
			if (ids.every((id) => resolved.has(id)) || Date.now() >= deadline) return ids.map((id) => resolved.get(id)).filter((trade) => trade !== void 0);
			await sleep(RESOLVE_TRADES_POLL_INTERVAL_MS);
		}
	}
	async resolveTransactionsHashes(response) {
		if (response.transactionsHashes && response.transactionsHashes.length > 0) return response;
		const tradeIDs = response.tradeIDs ?? [];
		if (tradeIDs.length === 0) return response;
		const transactionsHashes = (await this.waitForResolvedTrades(tradeIDs)).filter((trade) => trade.status.toUpperCase() !== FAILED_TRADE_STATUS).map((trade) => trade.transaction_hash).filter((hash) => Boolean(hash));
		if (transactionsHashes.length === 0) return response;
		return {
			...response,
			transactionsHashes
		};
