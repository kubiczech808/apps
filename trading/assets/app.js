          ...
          ${history.map((run) => {
            ...
            return `
              <tr class="${auditAvailable ? "scrape-run-row" : ""}" ${auditAvailable ? `data-scrape-run-audit="${escapeHtml(run.id || "")}" tabindex="0" role="button" aria-label="Open scraping audit for ${escapeHtml(formatDate(run.runAt || ""))}"` : ""}>
                ...
                <td data-label="New / updated" title="What this run actually did to the tradable catalogue: new is the markets that joined the active set, updated is the ones already in it that this scan re-read and retention kept. A market fetched but then dropped - for its tag, its horizon, or the 8000 cap - produced no candidate and is in neither. The line below is the net change, which is the same figure less whatever resolved or was evicted.">${formatInteger(run.newObservationCount) || "0"} / ${formatInteger(run.updatedObservationCount) || "0"}${run.netObservationCount == null ? "" : `<small class="table-secondary">catalogue ${Number(run.netObservationCount) >= 0 ? "+" : ""}${formatInteger(Number(run.netObservationCount))} to ${formatInteger(Number(run.activeObservationCountAfter || 0))}</small>`}</td>
